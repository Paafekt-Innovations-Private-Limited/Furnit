...
//        } else if let arr = output.featureValue(for: "var_2421")?.multiArrayValue {
        ...
        let stage1DetectionsFull = extractDetections(from: detArray, confThreshold: 0.5)
        let decodeEnd = Date()
        if self.debugMode {
...
//                if let arr = cropOutput.featureValue(for: "var_2421")?.multiArrayValue {
        ...
        let rawDetections = extractDetections(from: detArray, confThreshold: 0.5)
        let nmsStart = Date()
...
//                    if conf > confidenceThreshold {// Kishore
