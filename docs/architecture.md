...
| AI room creation | Flat preview immediately → GeoCalib/Depth Anything/RTMDet on first save → calibrated projective depth-surface USDZ | Immediate method picker → sampled EXIF-aware decode → flat full-photo GLB preview → calibrated projective depth-surface GLB on save |
