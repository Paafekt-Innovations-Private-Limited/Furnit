...
- [`rtmdet-swift-flow.svg`](../diagrams/rtmdet-swift-flow.svg) — RTMDet instance segmentation
  ("brain") in room viewers: brain default auto-segments primary; **text.viewfinder** toggles
  full-video identify/segment modes; transparent cutouts composite over the 3D room; Core ML raw-head
  decode → confidence-first NMS → mask affinity → pixel-union cutout.
...
3. Tap **text.viewfinder** (top-right while brain is active). Live camera preview should appear with cluster boxes.
