...
struct SharpRoomView: View {
    ...
    /// Bounds from the PLY actually displayed (WebGL / SparkJS path).
    private let parsedBounds: RoomBounds?

    init(plyURL: URL, allowSave: Bool = true, photoOrientation: PhotoOrientation = .portrait, savedRoomWidth: Float? = nil, savedRoomHeight: Float? = nil, savedRoomModel: USDZModel? = nil) {
        self.plyURL = plyURL
    ...
    /// Parsed bounds from the displayed PLY file.
    ...
    private var webGLAndGestureLayer: some View {
        ZStack {
            AntimatterSplatView(
                ...
                roomBounds: effectiveBounds.map { SIMD3<Float>($0.width, $0.height, $0.depth) },
                actualBounds: nil,
