...
An unconfirmed 2026-09-03 touch candidate keeps the projection field of view fixed during
one-finger turns and clamps yaw/pitch to the authored version-5 look envelope. This avoids coupling
small finger movements to automatic projection zoom while allowing both axes in portrait and
landscape. The Android Debug build passes; device testing must still confirm natural sensitivity and
that turns near the authored limits do not expose renderer-gray gaps or objectionable photo edges.
...
controlled Pixel 9a checks. For one-finger turns, verify all four directions respond without a
field-of-view jump or exaggerated movement; the 2026-09-03 fixed-FOV touch candidate remains
device-unconfirmed.
