/**
 * SHARP full pipeline — CPU / portable Part3–4 only (Part1+2 via moduleCacheLoadPart12Cpu).
 */
...
jfloatArray runSharpFullPipeline_Cpu(
        JNIEnv* env,
        jobject thiz,
...
if (!modelDirPath || !imageNCHW) {
    ...
    // CPU batch-4 can SIGSEGV (null deref) on some devices — use single when part12ForceSingle.
    ...
    LOGD("0.25x patch: %lldms. All %d patches: %lldms", nowMs() - t025, limit1x + limit05x + 1, nowMs() - t1x);
        ...
        // Fill x1Feat and x2Feat from downsampled x0Feat when 0.5x/0.25x skipped.
        // Avoids foggy/grid artifacts from zeroed x1/x2 fed to Part4 decoder.
    ...
    const std::string path4a512 = pathJoin(modelDir, "sharp_split_part4a_chunk_512.pte");
    const std::string path4a65 = pathJoin(modelDir, "sharp_split_part4a_chunk_65.pte");
    ...
    LOGI("SHARP_pte_paths useVulkan=0 dir=%s\n  Part3=%s\n  Part4a512=%s\n  Part4a65=%s\n  Part4b_single=%s",
         modelDir.c_str(),
         path3.c_str(),
         path4a512.c_str(),
         path4a65.c_str(),
    ...
    if (progressReporter && reportProgressMethodId) {
        reportProgress(env, progressReporter, reportProgressMethodId, 0.62f, "Part 4a (1/2): ViT 512 on CPU…");
    }
    ...
    std::atomic<bool> hbKeep512{true};
    std::thread hb512(cpuForwardHeartbeatLoop, &hbKeep512, "Part4a/512", 10);
    auto out4a512 = mod4a512->forward(in4a512);
    hbKeep512.store(false, std::memory_order_release);
    if (hb512.joinable()) hb512.join();
    ...
    std::atomic<bool> hbKeep65{true};
    std::thread hb65(cpuForwardHeartbeatLoop, &hbKeep65, "Part4a/65", 10);
    auto out4a65 = mod4a65->forward(in4a65);
    hbKeep65.store(false, std::memory_order_release);
    if (hb65.joinable()) hb65.join();
    ...
    // ── Part4b: Stable ON → single on disk in modelDir first (JNI passes preferSingle from Settings only);
    // then batched tiled → seq tiled → single fallback when Stable OFF or no early single file.
    ...
    if (useSinglePart4bOnly && path4bSingle.empty()) {
        LOGW(
                "Stable Part4b is ON but no single decoder in modelDir=%s "
                "(expected one of: sharp_split_part4b_int8.pte, sharp_split_part4b_fp16.pte, sharp_split_part4b.pte). "
                "Push one of these next to tile_b4, then reopen the app. Falling back to tiled Part4b (~100s+).",
                modelDir.c_str());
    }
    ...
    if (useSinglePart4bOnly && !path4bSingle.empty()) {
        ...
        if (mod4b->load() != Error::Ok) {
            LOGE("Part4b single load fail: %s — trying tiled Part4b", path4b.c_str());
        } else {
    ...
    if (part4bSingleEarlyAttempted) {
        env->ReleaseFloatArrayElements(imageNCHW, imageData, JNI_ABORT);
        LOGE("Part4b: tiled paths failed after Stable single attempt (check modelDir=%s)", modelDir.c_str());
        return nullptr;
    }

    // 4b(c): single Part4b fallback (Stable OFF: prefer tiled first; end here if no tiles worked)
    long long tP4b = nowMs();
