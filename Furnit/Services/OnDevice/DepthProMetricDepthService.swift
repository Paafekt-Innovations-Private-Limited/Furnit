...
/// Runs the bundled Depth Anything V2 Metric Indoor model and writes a metric depth buffer
/// next to the generated room so `WallMeasurementEstimator` can consume it during calibration.
@MainActor
final class DepthProMetricDepthService: ObservableObject {
    ...
    /// Apache-2.0 path only. Do not add Apple Depth Pro candidates here unless its commercial terms are verified.
    ...
    nonisolated private static func metricDepthURL(forRoomURL roomURL: URL) -> URL {
        ...
        return roomFolder.appendingPathComponent("\(stem)_depthanything_metric_depth.bin")
    }
    ...
    nonisolated private static func runPrediction(
        ...
        if let metricFeature = findMetricDepthOutput(in: output) {
            ...
            let exactFocal = exactFocalLengthPx(sidecar: sidecar, imageWidth: sourceSize.width)
            let focal = exactFocal?.value ?? 0
            let focalSource = exactFocal?.source ?? "DEPTH_ANYTHING_MODEL_ONLY"
            let fovDegrees = exactFocal.map { focalLengthPxToFovDegrees(focalPx: $0.value, imageWidth: sourceSize.width) } ?? 0
        ...
        throw DepthProError.invalidOutput("DEPTH ANYTHING METRIC DEPTH OUTPUT NOT FOUND")
    }
