...
path generates dimensions from **GeoCalib + Depth Anything + RTMDet object-anchor** inference before
preview (see `DepthAnythingRoomReconstructor.reconstructWithResult`). The preview opens the final
projective USDZ with measured metadata, and Save promotes that same artifact.
...
|-------|------|--------------|
| **Preview** | After the AI generation pass | Measured from GeoCalib + Depth Anything + RTMDet (`reconstructWithResult`) → projective USDZ + metadata |
| **Saved USDZ** | After user taps Save | Byte-preserved preview artifact and the same measured metadata |
