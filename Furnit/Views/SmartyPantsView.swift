...
final class SmartyPantsContainerView: UIView, AVCaptureVideoDataOutputSampleBufferDelegate, UIGestureRecognizerDelegate {
    ...
    private struct Track {
        ...
        // NEW — best mask stored in proto resolution
        var bestMask: [Float]?          // length = Hp * Wp
        var bestMaskConfidence: Float   // track best detection confidence
    }
    ...
    private let memoryBboxShrink: Float = 0.85         // Shrink memory region to object core
    ...
    deinit {
        ...
        // Clean up mask memory and tracking data
        trackMaskMemory.removeAll()
        trackPreviousCenter.removeAll()
...
//    }
    ...
    private func processFrame(_ pixelBuffer: CVPixelBuffer) {
        ...
        // frameIndex += 1
        // updateTracks(with: uniqueDetections, frameIndex: frameIndex)
    ...
    // MARK: - Bbox Intersection Helper
    private func bboxIntersection(_ a: DetectionSmarty, _ b: DetectionSmarty, scaleMask: Float) -> (x1: Int, y1: Int, x2: Int, y2: Int)? {
        let aLeft = a.x - a.width * 0.5
        let aRight = a.x + a.width * 0.5
        let aTop = a.y - a.height * 0.5
        let aBottom = a.y + a.height * 0.5
        ...
        // No intersection if any dimension is invalid
        guard ix2 > ix1 && iy2 > iy1 else { return nil }
        
        // Convert to mask coordinates
        let mx1 = max(0, Int(ix1 * scaleMask))
        let my1 = max(0, Int(iy1 * scaleMask))
        let mx2 = Int(ix2 * scaleMask)
        let my2 = Int(iy2 * scaleMask)
        
        return (x1: mx1, y1: my1, x2: mx2, y2: my2)
    }
    
    // MARK: - Shrunk Memory Region Helper
    private func getShrunkMemoryRegion(for detection: DetectionSmarty, scaleMask: Float, Wp: Int, Hp: Int) -> (x1: Int, y1: Int, x2: Int, y2: Int)? {
        let scale = scaleMask
        
        // Get full bbox
        let fullX1 = max(0, (detection.x - detection.width / 2) * scale)
        let fullY1 = max(0, (detection.y - detection.height / 2) * scale)
        let fullX2 = min(Float(Wp), (detection.x + detection.width / 2) * scale)
        let fullY2 = min(Float(Hp), (detection.y + detection.height / 2) * scale)
        
        guard fullX2 > fullX1 && fullY2 > fullY1 else { return nil }
        
        // Shrink to object core using linear interpolation
        let shrinkFactor = (1.0 - memoryBboxShrink) / 2.0  // 0.075 for each side
        let shrunkX1 = fullX1 + (fullX2 - fullX1) * shrinkFactor
        let shrunkY1 = fullY1 + (fullY2 - fullY1) * shrinkFactor
        let shrunkX2 = fullX2 - (fullX2 - fullX1) * shrinkFactor
        let shrunkY2 = fullY2 - (fullY2 - fullY1) * shrinkFactor
        
        return (
            x1: max(0, Int(shrunkX1)),
            y1: max(0, Int(shrunkY1)),
            x2: min(Wp, Int(shrunkX2)),
            y2: min(Hp, Int(shrunkY2))
        )
    }
    
    // MARK: - Simple IoU-based tracking (with best confidence logic)
    private func updateTracks(with detections: [DetectionSmarty], frameIndex: Int) {
        ...
        for pair in pairs {
            ...
            let det = detections[pair.detIndex]
            ...
            // ⬅️ UPDATE BEST CONFIDENCE (mask updated later inside generateCutoutTwoStage)
            if det.confidence > t.bestMaskConfidence {
                t.bestMaskConfidence = det.confidence
                // bestMask itself will be updated after rawMask is computed in generateCutoutTwoStage
            }
        ...
        // EXISTING TRACKS – MISSED OR UPDATED
...
//    }
    ...
    private func generateCutoutTwoStage(
        ...
        autoreleasepool {
            ...
            // 3) Build union of detection rects in output image coords (same as before)
            ...
            // Row-wise interval merge (unchanged)
            ...
            for y in 0..<height {
                ...
                // If no detection covers this row, zero the entire row
                ...
                while x < width {
                    ...
                    // Transparent gap before next interval
                    ...
                    // Inside interval: use alphaBytes
