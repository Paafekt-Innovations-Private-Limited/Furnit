...
| Diagram | Flow |
|---|---|
| [`room-generation-flow.svg`](room-generation-flow.svg) | **Photo → 3D** — home toolbar → `SinglePhotoRoomView` collects a camera/gallery image and sidecar metadata, `DepthAnythingRoomReconstructor` runs GeoCalib + Depth Anything + RTMDet object-anchor work, room measurement produces W×H×D, then the app exports a textured USDZ and opens a room viewer with inline brain segmentation. |
| [`rtmdet-swift-flow.svg`](rtmdet-swift-flow.svg) | RTMDet in room viewers — brain button starts `segmentPrimary` (auto primary cutout); **text.viewfinder** toggles full-video `identifyOnly` (live preview + cluster boxes) and `segmentSelected` (transparent cutout over 3D room). Core ML image input → raw heads → NMS → mask affinity → pixel-union cutout → overlay gestures. |
