...
/** Apply NDC correction to packed Gaussian tile output. */
static void correctNDC(float* data, int numGaussians, int tileRow, int tileCol) {
...
static int runOneTile(
        ...
        float* outputDst,
        int tileRow, int tileCol, int tileIdx) {
    ...
    correctNDC(outputDst, numGaussians, tileRow, tileCol);
...
Java_com_furnit_android_services_ExecutorchInt8Sharp_runTiledPart4bNative(
    ...
    LOGD("Starting native tiled Part4b: model=%s threads=%d parallel=%d",
         modelPath.c_str(), numThreads, parallelTiles);
    ...
    // img: 3*(384*384)=442368, lat0/lat1/x0: 1024*(24*24)=589824, x1: 1024*(12*12)=147456, x2/xl: 1024*(6*6)=36864
    ...
    // Release JNI arrays (we have copies in crops[])
    ...
    // Native parallel (2 modules, 2 threads) can OOM or hit ExecuTorch/XNNPACK thread-safety issues.
    // Run sequential in native until verified; Java "parallel" pref is ignored for stability.
    const bool useParallel = false;  // parallelTiles && false → always sequential in C++

    if (parallelTiles && useParallel) {
        // 2-module parallel: load two separate Module instances, process tiles in pairs
        ...
        if (errA != Error::Ok || errB != Error::Ok) {
            ...
            for (int pairStart = 0; pairStart < NUM_TILES && success; pairStart += 2) {
                ...
                auto tPair = std::chrono::steady_clock::now();
                ...
                if (resultA != 0 || (idxB >= 0 && resultB != 0)) {
                    ...
                    auto pairMs = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::steady_clock::now() - tPair).count();
                    LOGD("Tiles %d+%d: %d Gaussians/tile, %lldms (parallel)",
                         idxA, idxB, GAUSSIANS_PER_TILE, (long long)pairMs);
                }
            }
        }
    } else {
        ...
        // Sequential: load model once, reuse for all 16 tiles
    ...
    // Single copy: native heap → Java heap (the only JNI copy in the whole pipeline)
