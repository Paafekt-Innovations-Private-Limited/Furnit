...
/// Renderer type for Gaussian splat viewing
enum SplatRendererType: String, CaseIterable {
    case metal = "Metal"
    case webgl = "WebGL"
}

/// Simple 3D Gaussian Splat viewer using MetalSplatter or WebGL
...
struct SharpRoomView: View {
    ...
    /// Toggle between Metal and WebGL renderer
    @State private var selectedRenderer: SplatRendererType = .webgl

    var body: some View {
        ZStack {
            // Renderer view based on selection
            if selectedRenderer == .metal {
                // MetalSplatter view
                ...
                // WebGL antimatter15/splat view
                AntimatterSplatView(plyURL: plyURL, onLoaded: {
                    isLoading = false
                })
                .ignoresSafeArea()
            }
            ...
            if !showingSmartyPants {
                VStack {
                    HStack {
                        ...
                        Button(action: {
                            isLoading = true
                            selectedRenderer = selectedRenderer == .metal ? .webgl : .metal
                        }) {
                            HStack(spacing: 4) {
                                Image(systemName: selectedRenderer == .metal ? "cpu" : "globe")
                                    .font(.system(size: 14))
                                Text(selectedRenderer.rawValue)
                                    .font(.caption.bold())
                            }
                            .foregroundColor(.white)
                            .padding(.horizontal, 10)
                            .padding(.vertical, 6)
                            .background(Capsule().fill(selectedRenderer == .metal ? Color.orange.opacity(0.8) : Color.purple.opacity(0.8)))
                        }

                        // Recenter button (works for both renderers)
                        Button(action: {
                            if selectedRenderer == .metal {
            ...
            // Joystick at bottom center (only for Metal renderer)
            if selectedRenderer == .metal {
                VStack {
                    Spacer()
...
struct AntimatterSplatView: UIViewRepresentable {
    ...
    private func generateSplatViewerHTML() -> String {
        ...
        <body>
            <div id="progress"></div>
            <div id="message">Loading 3D room...</div>
            <div id="spinner"></div>
            <div id="fps"></div>
            <div id="camid"></div>
            <div id="info"></div>
            ...
            <script>
                // Override URL params to point to our local PLY
                ...
                function setupCamera() {
                    console.log('[setupCamera] Called, viewMatrix exists:', typeof viewMatrix !== 'undefined');
                    console.log('[setupCamera] carousel value:', typeof carousel !== 'undefined' ? carousel : 'undefined');

                    // Check if viewMatrix exists (set by main.js)
                    if (typeof viewMatrix !== 'undefined' && viewMatrix) {
                        // SHARP room is at Z: -3 to -1, centered at X:0, Y:0
                        // Camera at Z=3 looking toward -Z (front of room)
                        // This matrix: camera at (0, 0, 3) looking down -Z axis
                        viewMatrix = [
                            1, 0, 0, 0,      // right vector
                            0, 1, 0, 0,      // up vector
                            0, 0, 1, 0,      // forward vector (looking toward -Z)
                            0, 0, 3, 1       // position at z=3
                        ];

                        console.log('[setupCamera] viewMatrix set to front view at z=3');

                        // FORCE stop carousel - this is causing the floating!
                        if (typeof carousel !== 'undefined') {
                            carousel = false;
                            console.log('[setupCamera] Carousel DISABLED');
                        }
                ...
                // Stop carousel floating - runs continuously
                function stopCarousel() {
                    // Aggressively stop carousel every frame
                    setInterval(function() {
                        if (typeof carousel !== 'undefined' && carousel === true) {
                            carousel = false;
                            console.log('[stopCarousel] Carousel was true, set to false');
                        }
                    }, 100);

                    // Also stop on any touch
                    document.addEventListener('touchstart', function() {
                        if (typeof carousel !== 'undefined') {
                            carousel = false;
                            console.log('[touch] Carousel stopped');
                        }
                    }, true);
                    ...
                    console.log('[stopCarousel] Carousel stopper initialized');
                }

                // Timing
                const jsStartTime = performance.now();
                console.log('[WebGL JS] Script started');

                // Notify iOS when loaded
                window.addEventListener('load', function() {
                    const loadTime = performance.now() - jsStartTime;
                    console.log('[WebGL JS] Window load: ' + loadTime.toFixed(0) + 'ms');

                    // Stop carousel floating immediately
                    stopCarousel();
                    ...
                    const checkLoaded = setInterval(function() {
                        ...
                        // Try to fix camera once content is loading
                        if (checkCount === 20) { // After ~2 seconds
                            setupCamera();
                        }

                        if (message && message.style.display === 'none') {
                            clearInterval(checkLoaded);
                            setupCamera(); // Final camera setup

                            const totalTime = performance.now() - jsStartTime;
                            console.log('[WebGL JS] Splat visible: ' + totalTime.toFixed(0) + 'ms');
                        ...
                        if (checkCount > 150) { // 15 seconds timeout
                            clearInterval(checkLoaded);
                            setupCamera();
                            console.log('[WebGL JS] Timeout after 15s');
                        }
                    }, 100);
                });
            </script>
        ...
        @objc private func recenterCamera() {
            ...
            let js = """
                if (typeof viewMatrix !== 'undefined') {
                    ...
                    // Force stop carousel
                    if (typeof carousel !== 'undefined') carousel = false;
                    console.log('[Recenter] Camera reset to z=3, carousel stopped');
                }
            """
