...
/// YOLO wall segmentation + Depth Anything metric depth (on disk) + camera EXIF → front wall width/height in meters
...
enum WallMeasurementEstimator {
    ...
    static func measure(
        ...
        // --- Detector boxes (needed for depth + fallback dimensions) ---

        let detsRaw: [FurnitureFitDetection]
        do {
            detsRaw = try RTMDetImageInference.runInstanceSegmentation(
                image: thumbnail,
                model: model,
            ...
            logWallMeasurement("measure abort: detector \(error.localizedDescription)")
            return nil
        }

        let sourceSize = sourceImagePixelSize(for: thumbnail)
        let iw = sourceSize.width
        let ih = sourceSize.height
        logWallMeasurement("reference_image size=\(iw)x\(ih) detector=rtmdet class_score_floor=\(classScoreFloor)")
        ...
        logWallMeasurement("detector_detections count=\(mapped.count) sample=[\(sample)]")
        ...
        // --- Width / Height: detector pinhole (targets the wall surface) ---
        ...
        // The detector-picked rect projects to meters via pinhole camera.
    ...
    private static func metricDepthCandidateFileNames(roomURL: URL?) -> [String] {
        ...
        return [
            "\(stem)_depthanything_metric_depth.bin",
            "\(stem)_depthpro_metric_depth.bin",
        ] + metricDepthCandidateFileNames
    }
