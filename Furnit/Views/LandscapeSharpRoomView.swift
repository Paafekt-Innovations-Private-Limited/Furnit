...
struct LandscapeWebGLBoundaryManager {
    let width: Float
    let height: Float
    let depth: Float

    /// Default room dimensions when bounds not provided
    static let defaultBounds = LandscapeWebGLBoundaryManager(width: 4.0, height: 3.0, depth: 2.0)
...
struct LandscapeSharpRoomView: View {
    let plyURL: URL
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showShareSheet) {
            ...
            LandscapeShareSheet(activityItems: [threeDGSPlyURL])
        }
        .onAppear {
            loadMLModel()
        }
...
class LandscapeLocalFileSchemeHandler: NSObject, WKURLSchemeHandler {
    var plyURL: URL?
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        guard let requestURL = urlSchemeTask.request.url else {
            urlSchemeTask.didFailWithError(NSError(domain: "LandscapeLocalFileSchemeHandler", code: -1, userInfo: nil))
            return
        }

        let path = requestURL.path
        print("🏞️ LandscapeLocalFileSchemeHandler: Request for \(path)")
        ...
        if path == "/" || path == "/index.html" || path.isEmpty {
            ...
            print("🏞️ LandscapeLocalFileSchemeHandler: Served HTML (\(data.count) bytes)")
            return
        }
...
struct LandscapeAntimatterSplatView: UIViewRepresentable {
    let plyURL: URL
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        let schemeHandler = LandscapeLocalFileSchemeHandler()
        ...
        if let url = URL(string: "splat://local/") {
            ...
            print("🏞️ LandscapeAntimatterSplatView: Loading from custom scheme")
        }
    ...
    private func generateSplatViewerHTML(bounds: SIMD3<Float>?) -> String {
        ...
        let boundaryManager = LandscapeWebGLBoundaryManager(bounds: bounds)
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        var schemeHandler: LandscapeLocalFileSchemeHandler?
        let onLoaded: () -> Void
...
struct LandscapeWebGLJoystickOverlay: View {
...
struct LandscapeShareSheet: UIViewControllerRepresentable {
...
#Preview {
    NavigationStack {
        ...
        LandscapeSharpRoomView(plyURL: URL(fileURLWithPath: "/sample.ply"))
    }
}
