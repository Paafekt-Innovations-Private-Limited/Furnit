...
| AI room creation | GeoCalib/Depth Anything/RTMDet → version-5 single-surface projective USDZ → exact preview → Save promotes the same artifact; fixed-FOV two-axis touch candidate (device-unconfirmed) | Immediate method picker → EXIF-aware decode → metric measurement → one continuous version-5 projective GLB → exact preview → byte-identical Save promotion; flat fallback only on generation failure; fixed-FOV two-axis touch candidate (device-unconfirmed) |
...
| Bundled samples | Shared-source USDZ data assets; Scandinavian opens portrait, Industrial opens landscape; volumetric opening camera is derived from model bounds and live viewport (device-unconfirmed) | Shared-source GLBs → background staging/validation → `GLBRoomActivity`; same orientations and bounds/viewport-derived opening camera (device-unconfirmed) |
